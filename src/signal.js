...
class Signal {
  ...
  switchToLatest() {
    return new Signal(s => {
      ...
      let d = con.subject
        .bind(v => v.takeUntil(con.subject).concat(Signal.never()))
        ._subscribeProxy(s)
      let d2 = con.connect()
      return new Disposable(() => {
        d.dispose()
  ...
  repeat(count) {
    ...
    return new Signal(ss => {
        ...
        if (n > 0) {
          d.addDisposable(
            this._subscribeProxy(
              new Subscriber(sendNext, sendComplete, sendError)
            )
          )
        } else {
          ss.sendComplete()
      ...
      d.addDisposable(
        this._subscribeProxy(new Subscriber(sendNext, sendComplete, sendError))
      )
      return d
    })
  }
